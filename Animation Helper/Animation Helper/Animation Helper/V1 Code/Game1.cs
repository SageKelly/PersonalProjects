...
namespace Animation_Helper_V_1_0_0
{
    //PROBLEMS:
    /*
     * 
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        * change colors in an effort to reveal dangling pixels.
        ...
        string sDefaultDirFile = @"Assets\DefaultDir.txt";

        //This is the location of the directory stack file.
        string sStackFile = @"Assets\FileStack.txt";
        ...
        public Game1()
        {
            ...
            graphics.PreferredBackBufferHeight = 600; //DO NOT CHANGE!!!
            graphics.PreferredBackBufferWidth = 800;//DO NOT CHANGE!!!


            ViewportRect = new Rectangle(0, iHEADER_SPACING, graphics.PreferredBackBufferWidth,
                graphics.PreferredBackBufferHeight - (iHEADER_SPACING * 2));
        ...
        protected override void LoadContent()
        {
            ...
            else if (AS == AnimStates.LoadingElems)
            {
                ...
                if (SelectedPic.Bounds.Width > graphics.PreferredBackBufferWidth)
                    GlobalViewRect = new Rectangle(0, (graphics.PreferredBackBufferHeight / 2) + (graphics.PreferredBackBufferHeight / 4),
                        graphics.PreferredBackBufferWidth, SelectedPic.Height);
                else
                    GlobalViewRect = new Rectangle(0, (graphics.PreferredBackBufferHeight / 2) + (graphics.PreferredBackBufferHeight / 4),
            ...
            else
            {
                ...
                PicGetter = new FileStream(@"Assets\FolderBar.png", FileMode.Open);
                FolderBar = Texture2D.FromStream(this.GraphicsDevice, PicGetter);

                PicGetter = new FileStream(@"Assets\HighlightBar.png", FileMode.Open);
                SelectionBar = Texture2D.FromStream(this.GraphicsDevice, PicGetter);


                PicGetter = new FileStream(@"Assets\Up Arrow.png", FileMode.Open);
                UpArrow = Texture2D.FromStream(this.GraphicsDevice, PicGetter);


                PicGetter = new FileStream(@"Assets\Down Arrow.png", FileMode.Open);
                DownArrow = Texture2D.FromStream(this.GraphicsDevice, PicGetter);


                PicGetter = new FileStream(@"Assets\Right Arrow.png", FileMode.Open);
                RightArrow = Texture2D.FromStream(this.GraphicsDevice, PicGetter);


                PicGetter = new FileStream(@"Assets\Left Arrow.png", FileMode.Open);
                LeftArrow = Texture2D.FromStream(this.GraphicsDevice, PicGetter);

                PicGetter = new FileStream(@"Assets\Selection Border.png", FileMode.Open);
                SelectionBorder = Texture2D.FromStream(this.GraphicsDevice, PicGetter);

                PicGetter = new FileStream(@"Assets\White BG.png", FileMode.Open);
            ...
            switch (AS)
            {
                ...
                case AnimStates.FolderSelection:
                    ...
                    spriteBatch.DrawString(font, "Open Folder", new Vector2(
                        0, graphics.PreferredBackBufferHeight - iTEXT_SPACING - (iTEXT_SPACING / 2)), TextColor);

                    spriteBatch.DrawString(font, "Select", new Vector2(
                        graphics.PreferredBackBufferWidth / 2,
                        graphics.PreferredBackBufferHeight - iTEXT_SPACING - (iTEXT_SPACING / 2)), TextColor);
                     ...
                     */
                    spriteBatch.Draw(FolderBar, new Vector2(iFolderDecision * (graphics.PreferredBackBufferWidth / 2),
                        graphics.PreferredBackBufferHeight - iTEXT_SPACING - (iTEXT_SPACING / 2) + 4), Color.White);
                    //-------------------------------------------------------------------------------------------------------------------
                    #endregion
                ...
                case AnimStates.AnimationModding:
                    ...
                    //Draw this on the back layer to ensure it draws before the actual picture. This will change colors to reveal "floating pixels"
                    spriteBatch.Draw(WhiteBG, new Vector2(graphics.PreferredBackBufferWidth / 4, iHEADER_SPACING),
                        AnimRect, BG_Color, 0, Vector2.Zero, fPicScale, SpriteEffects.None, 1);
                    //the actual picture
                    spriteBatch.Draw(SelectedPic, new Vector2(graphics.PreferredBackBufferWidth / 4, iHEADER_SPACING),
                        AnimRect, Color.White, 0, Vector2.Zero, fPicScale, SpriteEffects.None, 0);
                    spriteBatch.Draw(SelectedPic, new Vector2(0, (graphics.PreferredBackBufferHeight / 2) + (graphics.PreferredBackBufferHeight / 4)),
                    ...
                    switch (Focus)
                    {
                        ...
                        case Focuses.FPS:
                            ...
                            if (Iinput.KBState.IsKeyDown(Keys.Down))
                            {
                                ...
                                if (DownButtonDownTime.TotalMilliseconds >= iDELAY)
                                {
                                    VIncOffset.X = 1;
                                    iTimeSwitch = iSLOW_INC;
                                    if (DownButtonDownTime.TotalMilliseconds >= (iDELAY * 3))
                                    {
                                        VIncOffset.X = 2;
                                        iTimeSwitch = iMED_INC;
                        ...
                        case Focuses.ViewSize:
                            ...
                            if (Iinput.KBState.IsKeyUp(Keys.Up) && Iinput.PrevKBState.IsKeyDown(Keys.Up))
                            {
                                UpButtonDownTime = new TimeSpan(0);
                            }
                            #endregion
                            if (Iinput.KBState.IsKeyDown(Keys.Down) && Iinput.PrevKBState.IsKeyUp(Keys.Down))
                            {
                            ...
                            if (Iinput.KBState.IsKeyUp(Keys.Down) && Iinput.PrevKBState.IsKeyDown(Keys.Down))
                            {
                                DownButtonDownTime = new TimeSpan(0);
                            }
                            #endregion
                            if (Iinput.KBState.IsKeyDown(Keys.Left) && Iinput.PrevKBState.IsKeyUp(Keys.Left))
                            {
                            ...
                            if (Iinput.KBState.IsKeyDown(Keys.Left))
                            {
                                ...
                                if (LeftButtonDownTime.TotalMilliseconds >= iDELAY)
                                {
                                    if (LeftButtonDownTime.TotalMilliseconds >= (iDELAY * 3))
                                    {
                            ...
                            if (Iinput.KBState.IsKeyUp(Keys.Left) && Iinput.PrevKBState.IsKeyDown(Keys.Left))//If you've let go of the key
                            {
                                LeftButtonDownTime = new TimeSpan(0);
                            }
                            #endregion
                            if (Iinput.KBState.IsKeyDown(Keys.Right) && Iinput.PrevKBState.IsKeyUp(Keys.Right))
                            {
        ...
        public void CheckLRInput(GameTime gameTime,
            ButtonEventHandler DirMethod)
        {
              /*
        ...
        int 
            ...
            if (Iinput.KBState.IsKeyDown(Keys.Down))
            {
                ...
                if (ButtonIncTime.TotalMilliseconds >= iSLOW_INC)
                {
                    DirMethod(VIncOffset, 'D');
                    if (bTech_offset_on)
                        iTechnical_offset.Y -= VIncOffset.Y;
                    else
                        Offset.Y -= VIncOffset.Y;
                    ButtonIncTime = TimeSpan.Zero;
                }
            }
            if (Iinput.KBState.IsKeyUp(Keys.Down) && Iinput.PrevKBState.IsKeyDown(Keys.Down))
            ...
            if (Iinput.KBState.IsKeyDown(Keys.Left))
            {
                ...
                if (ButtonIncTime.TotalMilliseconds >= iSLOW_INC)
                {
                    DirMethod(IncOffset, 'L');
                    if (bTech_offset_on)
                        iTechnical_offset.X -= IncOffset.X;
                    else
                        Offset.X -= IncOffset.X;
                    ButtonIncTime = TimeSpan.Zero;
                }
            }
            if (Iinput.KBState.IsKeyUp(Keys.Left) && Iinput.PrevKBState.IsKeyDown(Keys.Left))
